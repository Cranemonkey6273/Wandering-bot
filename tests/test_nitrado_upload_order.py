...
class ProtectedXmlUploadOrderTests(unittest.TestCase):
    def setUp(self):
        ...
        self.original_staged_ftp = bot.upload_protected_dayz_xml_to_nitrado_ftp_staged
    ...
    def tearDown(self):
        ...
        bot.upload_protected_dayz_xml_to_nitrado_ftp_staged = self.original_staged_ftp
    ...
    def test_protected_xml_uses_staged_ftp_when_api_fails_before_write(self):
        def api_upload(*_args):
            self.calls.append("api")
            return False, "Nitrado API token or service ID is missing."

        def staged_upload(*_args):
            self.calls.append("staged")
            return True, "Renamed verified temporary upload into place via ukln138.gamedata.io."
        ...
        bot.upload_text_file_to_nitrado_api = api_upload
        bot.upload_protected_dayz_xml_to_nitrado_ftp_staged = staged_upload
        bot.verify_uploaded_protected_dayz_xml_text = verify

        ok, message = bot.upload_text_file_to_nitrado({}, "/dayzxb_missions/dayzOffline.enoch/cfgeventspawns.xml", SPAWNS_XML)

        self.assertTrue(ok)
        self.assertIn("staged FTP fallback was used", message)
        self.assertEqual(["api", "staged"], self.calls)
