...
- File relationship map: explain which files must be changed together. For example, event names must match between `events.xml` and `cfgeventspawns.xml`; custom ObjectSpawner, spawn-gear and restricted-area JSON files must be referenced from the correct `cfggameplay.json` array; and building loot depends on matching categories/usages across `types.xml`, limits definitions and map groups.
- Common-error help: explain XML closing tags, JSON commas/quotes/comments, coordinates, classnames, map/version differences and why the newest RPT after the change is the only useful error log.
- DayZ AI hand-off: Ultimate users can open the same permission-checked AI conversation from the app for file explanations, error diagnosis and validated drafts. Full raw-file editing stays desktop-first; the app presents the advice, validation, diff, download/share and a clearly warned route to the full dashboard.
